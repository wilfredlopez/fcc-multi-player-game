...
class Player {
  ...
  draw(context, coin, { mainPlayerImg, otherPlayerImg }, currPlayers) {
    const currDir = Object.keys(this.movementDirection).filter((dir) =>
    ...
    if (this.isMain) {
      ...
      context.drawImage(mainPlayerImg, this.x, this.y);
    } else {
      context.drawImage(otherPlayerImg, this.x, this.y);
    }
  ...
  movePlayer(dir, speed) {
    if (dir === "up") {
      this.y - speed >= calculations.playFieldMinY
        ? this.y -= speed
        : this.y -= 0;
    }
    if (dir === "down") {
      this.y + speed <= calculations.playFieldMaxY
        ? this.y += speed
        : this.y += 0;
    }
    if (dir === "left") {
      this.x - speed >= calculations.playFieldMinX
        ? this.x -= speed
        : this.x -= 0;
    }
    if (dir === "right") {
      this.x + speed <= calculations.playFieldMaxX
        ? this.x += speed
        : this.x += 0;
    }
  }
  ...
  calculateRank(arr) {
    ...
    const mainPlayerRank = this.score === 0
      ? arr.length
